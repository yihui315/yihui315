...
| `l1-governance-reflector` | validation helper | active | 9/10 | `scripts/reflect-l1-governance-loop.ps1` | Codex | Converts health, feedback, and state into advisory reflection JSON/Markdown with recoverability and strategy fields |
| `l1-loop-state-updater` | validation helper | active | 9/10 | `scripts/update-l1-loop-state.ps1` | Codex | Applies stop conditions, preserves extended state, and allows bounded auto-retry for recoverable failures; uses estimated cost only |
